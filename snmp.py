...
def getRoutingTable(ipRouter,router = None):
    session = Session(hostname=ipRouter, community=COMMUNITY, version=2)    #crear sessio per connectar amb el router
    networks = session.walk('IP-FORWARD-MIB::ipCidrRouteDest')              #xarxes desti
    nexthop = session.walk('IP-FORWARD-MIB::ipCidrRouteNextHop')            #nexthop per la xarxa desti
    tipus = session.walk('IP-FORWARD-MIB::ipCidrRouteType')                 #tipus de ruta
    masks = session.walk('IP-FORWARD-MIB::ipCidrRouteMask')                 #mascara de la xarxa desti

    table = list()

    for x, y, z, n in zip(networks, masks, nexthop, tipus):                 #combinem linea a linea la informacio rebuda
        route = RouteEntry(x.value,y.value,z.value)                         #creem un objecte ruta a partir de la informacio anterior
        ...
        table.append((x.value, y.value, z.value, tipus))                    #afegim la informacio anterior a la taula a mostrar
...
def getRouterInterfaces(ipRouter, routerName=None):
    global network
    session = Session(hostname=ipRouter, community=COMMUNITY, version=2)    #creem sessio a partir de la ip del router
    router = network.getRouter(routerName)                                  #recuperem objecte router a partir del nom
    ifaceIp = (session.walk('RFC1213-MIB::ipAdEntAddr'))                    #recuperem llista d'ip
    ...
    for ip in ifaceIp:
        ...
        table.append((ifaceName.value, ip.value, ifaceMask.value, speed, ifaceCost))        #afegim entrada a la taula a mostrar

        interface = Interface(ifaceName.value, ip.value, ifaceMask.value, speed, ifaceCost) #creem objecte interface amb tota la informacio necessaria
        router.addInterface(interface)                                                      #afegim aquesta interficie al router
        network.addIP(ip.value, routerName, ifaceMask.value)                                #afegim la ip de la interficie a la llista de ips per calcular el path posteriorment

    print "Interfaces: "
    headers = ["Name", "IP", "Mask", "Speed(Mbps)", "Cost"]
    print tabulate(table, headers, tablefmt="fancy_grid")
    getRoutingTable(ipRouter,router)                                                        #continuem el proces d' obtencio d'informacio
...
def getNeighbourAdress(ipRouter, lastRouter=None):
    ...
    router = network.getRouter(name.value)                                                  #recuperem objecte router en cas que existeixi...
    if router is None:
        router = Router(name.value)                                                         #si no existeix l' inicialitzem

    if lastRouter is not None:                                                              #si venim des d' un router...
        lastRouter.addAdjacentRouter(router)                                                #afegim l' adjacencia entre els routers que estem treballant

    if not network.routerExists(router):                                                    #si a la xarxa no existeix el router actual...
        ...
        neighbours = (session.walk('OSPF-MIB::ospfNbrIpAddr'))                              #recuperem les IP dels adjacents i tornem a reproduir el proces per a cada un
        for x in neighbours:
            getNeighbourAdress(x.value, router)                                             #crida recursiva a l' obtencio de la informacio
...
def generateGraph():
    ...
    for x in network.getRouters():                                      #per cada un dels routers...
        for y in x.getAdjacents():                                      #per cada un dels adjacents...
            head, tail = ipsSameNetwork(x, y)                           #mirem si hi ha alguna interficie en comu entre aquests i recuperem les IPs i la seva velocitat
            g1.edge(str(x), str(y), headlabel=head, taillabel=tail)     #afegim la interficie al graf
...
def ipsSameNetwork(r1, r2):
    ...
    for x in r1.getInterfaces():                                                            #per cada interficie del router1...
        ...
        for y in r2.getInterfaces():                                                        #per cada interficie del router2...
            ...
            if IPNetwork(str(ip1 + '/' + mask1)) == IPNetwork(str(ip2 + '/' + mask2)):      #comparem la ip1+mascara1 amb ip2+mascara2 per saber si son les interficies que uneixen
...
def findBestRoutes():
    ...
    for ipstart, ipend in itertools.permutations(allIP, 2):
        if ipstart[1] != ipend[1]:
            ...
            nexthop = router.getNexthop(ipend[0],ipend[2])                      #recuperem el nexhop a partir de la ip desti i la seva mascara
            toPrint = ipstart[0]                                                #incialitzem el path amb la ip inicial
            while(True):
                if(nexthop == '0.0.0.0'):                                       #si el nexthop es 0.0.0.0 significa que hi estem directament connectats
                    toPrint += '-->' + ipend[0]                                 #per tant afegim la ip desti al path
                    break
                else:                                                           #sino...
                    router = network.getRouterbyIP(nexthop)                     #actualitzem el router a partir de la ip del nexthop
                    nexthop = router.getNexthop(ipend[0],ipend[2])              #recalculem el nexthop per a la ip desti a partir del router actual
                    toPrint += '-->' + router.getName()                         #afegim el router al path
            table.append((ipstart[0],ipend[0],toPrint))                         #afegim path a la taula per mostrar
